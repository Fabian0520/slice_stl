...
def slice_mesh(mesh, location=[0, 0, 0], direction=[0, 1, 0]):
    ...
    cross_section_df = (
        pd.DataFrame(cross_section.vertices, columns=columns)
        .reset_index(drop=True)
        .sort_values(by=(direction_str, "x"))
    )
...
def plot_slices(data, aspect_ratio=1):
    ...
    plot_range_all["min"]["x":"y"] = (
        plot_range_all["min"]["x":"y"] - d_all["x":"y"] * 0.05
    )
    plot_range_all["max"]["x":"y"] = (
        plot_range_all["max"]["x":"y"] + d_all["x":"y"] * 0.05
    )
    plot_range_all["min"]["z"] = plot_range_all["min"]["z"] - d_all["z"] * 0.2
    plot_range_all["max"]["z"] = plot_range_all["max"]["z"] + d_all["z"] * 0.2

    fig_all, ax_all = plt.subplots(
        2, 1, sharex=True, sharey=True, figsize=(12, 9)
    )  # Din A4 Größe in inch Landscape
    ...
    for analysis in data:
        ...
        ax_single[0].set_ylim(
            [plot_range_all[col]["z"] for col in plot_range_all.columns]
        )
        ax_single[0].set_ylabel("Z [mm]")
        ax_single[0].set_title(f"Schnitte durch die X-Z Ebene")
        ax_single[0].set_xlim(
            [plot_range_all[col]["x"] for col in plot_range_all.columns]
        )
        ...
        ax_single[1].set_ylim(
            [plot_range_all[col]["z"] for col in plot_range_all.columns]
        )
        ax_single[1].set_ylabel("Z [mm]")
        ax_single[1].set_title(f"Schnitte durch die Y-Z Ebene")
        ax_single[1].set_xlim(
            [plot_range_all[col]["y"] for col in plot_range_all.columns]
        )
        ...
        for n_slice in analysis.cross_section.columns.levels[0]:
            ...
            if n_slice == "010":
                ax_single[0].scatter(
                    analysis.cross_section[(n_slice, "x")],
                    analysis.cross_section[(n_slice, "z")],
                    s=0.1,
                )
                ...
                ax_single[1].scatter(
                    analysis.cross_section[n_slice]["y"],
                    analysis.cross_section[n_slice]["z"],
                    s=0.1,
                )
        ...
        fig2.tight_layout()
        output_name = (
            pathlib.Path.cwd().joinpath("output").joinpath(analysis.name + ".png")
        )
...
def plot_contour(data):
    ...
    fig.colorbar(cntr, ax=ax, label="Z [mm]")
    output_name = (
        pathlib.Path.cwd().joinpath("output").joinpath(data.name + "_contour.png")
    )
